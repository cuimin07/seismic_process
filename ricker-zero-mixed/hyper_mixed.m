function [d,h,t] = hyper_mixed(dt,f0,tmax,h,tau,v,amp,snr,L,r)
%HYPERBOLIC_EVENTS: A program to generate data containing hyperbolas.
%
%  [d] = hyper_mixed(dt,f0,tmax,h,tau,v,amp,snr,L,r);
%
%  IN   dt:        sampling interval in secs 采样间隔
%       f0:        central freq. of a Ricker wavelet in Hz 一个ricker子波中心频率
%       tmax:      maximun time of the simulation in secs 模拟的最大时间
%       h:         vector of offsets in meters 偏移矢量
%       tau,v,amp: vectors of intercept, rms velocities
%                  and amplitude of each linear event
%                  (v is in m/s and tau in secs) 每个线性事件的截距矢量，均方根速度与振幅
%       snr:       signal to noise ratio (max amplitude of the clean
%                  signal/max amplitude of the noise) 信噪比
...
%
%    [d,h,t] = hyper_mixed; imagesc(h,t,d);
%
%
